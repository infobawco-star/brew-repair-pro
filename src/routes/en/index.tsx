  ...
  return <PageShell locale="en">
    <section className="relative overflow-hidden pt-32 pb-16 md:pt-40 md:pb-24">
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 md:px-8 lg:grid-cols-2">
        ...
        <div className="card-glow overflow-hidden rounded-2xl border border-primary/25"><img decoding="async" src={heroMachine} alt="Technician inspecting a professional espresso machine" width={1280} height={960} className="size-full object-cover" /></div>
      </div>
    </section>
    <section className="mx-auto max-w-7xl px-5 py-16 md:px-8 md:py-24">
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-3">{services.map(s => <article key={s.title} className="group overflow-hidden rounded-2xl border border-border bg-card"><div className="aspect-[4/3] overflow-hidden"><img width={960} height={720} decoding="async" src={s.image} alt={s.title} loading="lazy" className="size-full object-cover transition-transform duration-500 group-hover:scale-105" /></div><div className="p-6"><s.icon className="size-6 text-primary"/><h3 className="mt-3 text-lg font-bold">{s.title}</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{s.desc}</p></div></article>)}</div>
    </section>
